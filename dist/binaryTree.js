 ...
 */
function buildBalancedTree(keys, start, end) {
    if (start > end)
        return null;
 ...
 */
function benchmarkBinaryTreeInsertion(keys) {
 ...
 * @returns The elapsed time in milliseconds.
 */
function benchmarkBalancedTreeConstruction(keys) {
    const startTime = perf_hooks_1.performance.now();
 ...
 * @returns The elapsed time in milliseconds.
 */
function benchmarkSetInsertion(keys) {
...
function binaryTreeTest() {
    ...
    console.log(`Balanced tree construction time: ${Math.round(balancedTreeTime)} ms`);
